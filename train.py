...
parser.add_argument(
    ...
    type=str,  # wikiart dataset crawled from https://www.wikiart.org/
    help="Directory path to a batch of style images",
)
parser.add_argument(
    "--vgg", type=str, default="./experiments/vgg_normalised.pth"
...
for i in tqdm(range(args.max_iter)):
    ...
    if (i + 1) % args.save_model_interval == 0 or (i + 1) == args.max_iter:
        ...
        torch.save(
            state_dict, "{:s}/transformer_iter_{:d}.pth".format(args.save_dir, i + 1)
        )
        ...
        for key in state_dict.keys():
            state_dict[key] = state_dict[key].to(torch.device("cpu"))
        torch.save(
            state_dict, "{:s}/decoder_iter_{:d}.pth".format(args.save_dir, i + 1)
        )
        state_dict = network.module.embedding.state_dict()
        for key in state_dict.keys():
            state_dict[key] = state_dict[key].to(torch.device("cpu"))
        torch.save(
            state_dict, "{:s}/embedding_iter_{:d}.pth".format(args.save_dir, i + 1)
        )
